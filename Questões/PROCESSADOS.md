...
- **Verificação final:** checado com `grep -rl "não corresponde ao enunciado" Papiloscopista/provas/` que zero arquivos em todo o banco (Ciências Forenses, Tecnologia, Biologia 004, Legislação etc.) ainda usam o template genérico — só o lote de Português (001-006) e Biologia (001-003) foi afetado por ele. As demais disciplinas foram importadas por um processo de curadoria diferente (`all_questions_dedup.json`) que já gerava explicação real por questão desde o início. Rodada de qualidade encerrada.
